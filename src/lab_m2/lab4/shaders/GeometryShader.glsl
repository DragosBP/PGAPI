    ...
    for (int j = 0; j < no_of_instances; j++) {
        for (int i = 0; i < no_of_generated_points - 1; i++) {
            ...
            if (surface_type == SURFACE_TYPE_ROTATION) {
                point_row1_a = rotateY(point_row1_a, max_rotate * instance[0] / no_of_instances);
                point_row1_b = rotateY(point_row1_b, max_rotate * instance[0] / no_of_instances);
                point_row2_a = rotateY(point_row2_a, max_rotate * (instance[0] + 1) / no_of_instances);
                point_row2_b = rotateY(point_row2_b, max_rotate * (instance[0] + 1) / no_of_instances);
            } else {
                point_row1_a = translateX(point_row1_a, max_translate * instance[0] / no_of_instances);
                point_row1_b = translateX(point_row1_b, max_translate * instance[0] / no_of_instances);
                point_row2_a = translateX(point_row2_a, max_translate * (instance[0] + 1) / no_of_instances);
                point_row2_b = translateX(point_row2_b, max_translate * (instance[0] + 1) / no_of_instances);
            }
