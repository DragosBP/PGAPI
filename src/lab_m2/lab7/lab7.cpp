...
void Lab7::ComputeInterpolatedRotation(aiQuaternion& out, float animationTime, const aiNodeAnim* pNodeAnim)
{
    ...
    float time0 = (float)pNodeAnim->mRotationKeys[rotationIndex].mTime;
    float time1 = (float)pNodeAnim->mRotationKeys[nextRotationIndex].mTime;
    ...
    const aiQuaternion& startRotation = pNodeAnim->mRotationKeys[rotationIndex].mValue;
    const aiQuaternion& endRotation = pNodeAnim->mRotationKeys[nextRotationIndex].mValue;
...
void Lab7::ComputeInterpolatedScaling(aiVector3D& out, float animationTime, const aiNodeAnim* pNodeAnim)
{
    ...
    float time0 = (float)pNodeAnim->mRotationKeys[scalingIndex].mTime;
    float time1 = (float)pNodeAnim->mRotationKeys[nextScalingIndex].mTime;
    float deltaTime = time1 - time0;
    ...
    out = startScale + factor * (endScale - startScale);
}
...
void Lab7::ComputeInterpolatedPosition(aiVector3D& out, float animationTime, const aiNodeAnim* pNodeAnim)
{
    ...
    float time0 = (float)pNodeAnim->mPositionKeys[positionIndex].mTime;
    float time1 = (float)pNodeAnim->mPositionKeys[nextPositionIndex].mTime;
    float deltaTime = time1 - time0;
    ...
    // Save the result in the out variable
    aiVector3D startPos = pNodeAnim->mPositionKeys[positionIndex].mValue;
    aiVector3D endPos = pNodeAnim->mPositionKeys[nextPositionIndex].mValue;

    out = startPos + factor * (endPos - startPos);
}
