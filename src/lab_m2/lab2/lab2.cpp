...
void Lab2::Init()
{
    ...
    for (int i = 0; i < 10; ++i)
    {
        ...
        float pos_x = Rand01() * 20 - 10;
        float pos_y = Rand01() * 3;
        float pos_z = Rand01() * 20 - 10;
        lightInfo.position = glm::vec3(pos_x, pos_y, pos_z);
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    for (auto& l : lights)
    {
        ...
        const float radiansPerSecond = glm::radians(6.0f);
    ...
    // Lighting pass
    {
        ...
        for (auto& lightInfo : lights)
        {
            ...
            modelMatrix = glm::scale(modelMatrix, glm::vec3(2 * lightInfo.radius));
