...
void Lab3::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_RIGHT_BRACKET) && shrink < 1.5)
    {
        shrink += 0.01f;
    }
    else if (window->KeyHold(GLFW_KEY_LEFT_BRACKET) && shrink > 0.7) {
        shrink -= 0.01f;
    }
...
void Lab3::OnKeyPress(int key, int mods)
{
    ...
    if (key == GLFW_KEY_MINUS && nrInstances > 0)
    {
        nrInstances--;
        nrInstances %= maxInstances;
    }
